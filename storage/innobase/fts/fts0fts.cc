                               ...
                               trx_t *trx)
{
  ...
  *doc_id= 0;
  ...
  if (error == DB_SUCCESS)
  {
    const auto& config_pairs= reader.config_pairs;
    if (config_pairs.size() == 1)
    {
      const std::string& value_str= config_pairs[0].second;
      int n_parsed= sscanf(value_str.c_str(), FTS_DOC_ID_FORMAT, doc_id);
