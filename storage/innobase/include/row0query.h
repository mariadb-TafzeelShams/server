...
#ifndef INNOBASE_ROW0QUERY_H
#define INNOBASE_ROW0QUERY_H
...
private:
  ...
  @return true if should continue processing, false to break */
  bool lookup_clustered_record(dict_table_t *table,
  ...
  @param     mtr            mini-transaction for version building
  @param     match_count counter for processed records
  @return true if should continue processing, false to break */
  bool process_record_with_mvcc(dict_table_t *table,
                                dict_index_t *index,
                                const rec_t *rec,
                                rec_offs *offsets,
                                RecordCallback& callback,
...
public:
  ...
  /** Commit the current modification mtr. Commits any pending page
  changes from an update flow and clears the active mtr state.
  @returns DB_SUCCESS if an mtr was active and is now committed,
  DB_ERROR if no active mtr */
  dberr_t commit_update() noexcept;

  /** Roll back the current modification mtr.
  Rollback is implemented as an mtr commit without persisting
  an update, because page-level undo is managed by the
  higher-level InnoDB machinery (undo logs).
  @return DB_SUCCESS if an mtr was active
  DB_ERROR if no active mtr */
  dberr_t rollback_update() noexcept;
  ...
  @return error code on failure */
  dberr_t read(dict_table_t *table, dtuple_t *tuple, page_cur_mode_t mode,
  ...
  dberr_t read_by_index(dict_table_t *table, dict_index_t *sec_index,
                        dtuple_t *search_tuple, page_cur_mode_t mode,
                        RecordCallback& callback) noexcept;
