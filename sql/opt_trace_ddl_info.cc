...
static int pre_parsing_check(json_engine_t *je, String *err_buf,
                             const char *err_msg);
...
public:
  ...
  int read_container(json_engine_t *je, String *err_buf) override
  {
    while (je->state != JST_ARRAY_END)
    {
      ...
      {
        err_buf->append(STRING_WITH_LEN("error reading rec_per_key value"));
        return 1;
      }
      ...
      if (conv_err)
      {
        err_buf->append(STRING_WITH_LEN("records must be a numeric value"));
        return 1;
      }
      ha_rows *records_ptr= (ha_rows *) thd->alloc(sizeof(ha_rows));
      *records_ptr= records;
      list_values->push_back(records_ptr);
...
class Read_list_of_tbl_context : public Read_container_value
{
  THD *thd;
  List<trace_table_context_read> *list_tbl_ctx;

public:
  Read_list_of_tbl_context(THD *thd_arg,
                           List<trace_table_context_read> *list_tbl_ctx_arg)
      : thd(thd_arg), list_tbl_ctx(list_tbl_ctx_arg)
  {
  }
  int read_container(json_engine_t *je, String *err_buf) override
  {
    ...
    while (1)
    {
      ...
      table_ctx->irc_list.empty();
      rc= parse_table_context(thd, je, err_buf, table_ctx);
      if (rc == 0)
        list_tbl_ctx->push_back(table_ctx);
      else
        break;
    }
    return rc;
...
class Read_list_of_idx_context : public Read_container_value
{
  THD *thd;
  List<trace_index_context_read> *list_idx_ctx;

public:
  Read_list_of_idx_context(THD *thd_arg,
                           List<trace_index_context_read> *list_idx_ctx_arg)
      : thd(thd_arg), list_idx_ctx(list_idx_ctx_arg)
  {
  }
  int read_container(json_engine_t *je, String *err_buf) override
  {
    ...
    while (1)
    {
      trace_index_context_read *idx_ctx= new trace_index_context_read();
      idx_ctx->list_rec_per_key.empty();
      rc= parse_index_context(thd, je, err_buf, idx_ctx);
      if (rc == 0)
        list_idx_ctx->push_back(idx_ctx);
      else
        break;
    }
    return rc;
...
class Read_list_of_range_context : public Read_container_value
{
  THD *thd;
  List<trace_range_context_read> *list_range_ctx;

public:
  Read_list_of_range_context(
      THD *thd_arg, List<trace_range_context_read> *list_range_ctx_arg)
      : thd(thd_arg), list_range_ctx(list_range_ctx_arg)
  {
  }
  int read_container(json_engine_t *je, String *err_buf) override
  {
    ...
    while (1)
    {
      ...
      rc= parse_range_context(thd, je, err_buf, range_ctx);
      if (rc == 0)
        list_range_ctx->push_back(range_ctx);
      else
        break;
    }
    return rc;
...
public:
  ...
  int read_container(json_engine_t *je, String *err_buf) override
  {
    ...
    while (je->state != JST_ARRAY_END)
    {
      if (json_read_value(je))
      {
        err_buf->append(STRING_WITH_LEN("error reading ranges value"));
        return 1;
      }
      ...
      if (res)
      {
        err_buf->append(STRING_WITH_LEN("un-escaping error of range element"));
        return 1;
      }
      list_ranges->push_back(
...
class Read_list_index_read_costs : public Read_container_value
{
  THD *thd;
  List<trace_irc_context_read> *list_range_ctx;

public:
  Read_list_index_read_costs(THD *thd_arg,
                             List<trace_irc_context_read> *list_range_ctx_arg)
      : thd(thd_arg), list_range_ctx(list_range_ctx_arg)
  {
  }
  int read_container(json_engine_t *je, String *err_buf) override
  {
    ...
    while (1)
    {
      ...
      rc= parse_index_read_cost_context(thd, je, err_buf, irc_ctx);
      if (rc == 0)
        list_range_ctx->push_back(irc_ctx);
      else
        break;
    }
    return rc;
...
/*
  fill the err_buf with err_msg if the parsing checks fail
 */
static int pre_parsing_check(json_engine_t *je, String *err_buf,
                             const char *err_msg)
{
...
static int parse_helper(json_engine_t *je, String *err_buf,
                        const char *err_msg, Read_named_member *array)
{
  if (int rc= pre_parsing_check(je, err_buf, err_msg))
    return rc;
                               ...
                               trace_table_context_read *table_ctx)
{
  ...
  Read_named_member array[]= {
    ...
    {"num_of_records", Read_ha_rows(&table_ctx->total_rows), false},
    {"read_cost_io", Read_double(&table_ctx->read_cost_io), false},
    {"read_cost_cpu", Read_double(&table_ctx->read_cost_cpu), false},
    {"indexes", Read_list_of_idx_context(thd, &table_ctx->index_list), true},
    {"list_ranges", Read_list_of_range_context(thd, &table_ctx->ranges_list), true},
    {"list_index_read_costs", Read_list_index_read_costs(thd, &table_ctx->irc_list), true},
    {NULL, Read_double(NULL), true}
  };

  return parse_helper(je, err_buf, err_msg, array);
}
...
static int parse_index_context(
    THD *thd,
    json_engine_t *je, String *err_buf,
    trace_index_context_read *index_ctx)
{
  ...
  Read_named_member array[]= {
    ...
    {"rec_per_key",
      Read_list_of_ha_rows(thd, &index_ctx->list_rec_per_key), false},
    {NULL, Read_double(NULL), true}
  };
...
/*
  ...
  @return
   ...
   -1  EOF
*/
static int parse_range_context(
    THD *thd,
    json_engine_t *je, String *err_buf,
    trace_range_context_read *range_ctx)
{
  ...
  Read_named_member array[]= {
    ...
    {"num_rows", Read_ha_rows(&range_ctx->num_rows), false},
    {"cost", Read_range_cost_estimate(thd, &range_ctx->cost), false},
    {"max_index_blocks", Read_ha_rows(&range_ctx->max_index_blocks), false},
    {"max_row_blocks", Read_ha_rows(&range_ctx->max_row_blocks), false},
    {NULL, Read_double(NULL), true}
  };
...
/*
  ...
  @return
   ...
   -1  EOF
*/
static int parse_index_read_cost_context(
  THD *thd,
    json_engine_t *je, String *err_buf, trace_irc_context_read *irc_ctx)
{
  const char *err_msg= "Expected an object in the index_read_costs array";
 
  Read_named_member array[]= {
    {"key_number",       Read_uint(&irc_ctx->key), false},
    {"num_records",      Read_ha_rows(&irc_ctx->records), false},
    {"eq_ref",           Read_bool(&irc_ctx->eq_ref), false},
    ...
    {"max_index_blocks",
      Read_longlong(&irc_ctx->cost.max_index_blocks), false},
    {"max_row_blocks",   Read_longlong(&irc_ctx->cost.max_row_blocks), false},
...
*/
bool Optimizer_context_replay::load_range_stats_into_client(
...
bool Optimizer_context_replay::parse()
{
  ...
  Read_named_member array[]= {
    ...
    {"list_contexts", Read_list_of_tbl_context(this->thd, &this->ctx_list),
      false},
    {NULL, Read_double(NULL), true}
  };
  ...
  while (trace_table_context_read *tbl_ctx= table_itr++)
  {
    ...
    for (trace_irc_context_read *irc= irc_itr++; irc; irc= irc_itr++)
    {
      ...
      DBUG_PRINT("info", ("eq_ref: %d", irc->eq_ref));
      {
        ...
        DBUG_PRINT("info", ("max_index_blocks: %llx", irc->cost.max_index_blocks));
