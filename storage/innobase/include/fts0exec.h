...
public:
  ...
  /** Full scan of CONFIG table with callback.
  @return DB_SUCCESS or error code */
  dberr_t read_all_config(RecordCallback& callback) noexcept;

  /** Read the key from the config table by key
  and process the matching record with callback
  @param key 		config key
  @param callback	callback to process the record
  @return DB_SUCCESS or error code */
  dberr_t read_config(const char *key, RecordCallback& callback) noexcept;
